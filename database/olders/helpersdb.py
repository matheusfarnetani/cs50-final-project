...
def search_tables(card=None, date=None, time=None, card_type=None, local=None):
    ...
    query = db.session.query(
        ...
        models.Registers.hours,
        models.Registers.minutes,
        models.Registers.seconds,
        models.Equipments.description.label('equipment'),
        models.Places.name.label('local')
    ).join(models.RegisterHasCard).join(models.Cards).join(models.Register).join(
        models.RegisterHasEquipment).join(models.Equipments).join(models.Places)

    # Add conditions based on user input
    if card:
        query = query.filter(models.Cards.uid.like(f"%{card}%"))
    if date:
        query = query.filter(models.Registers.date == date)
    if time:
        query = query.filter(
            models.Registers.hours == time.hour,
            models.Registers.minutes == time.minute,
            models.Registers.seconds == time.second
        )
    if card_type:
        query = query.filter(models.Cards.type == card_type)
    if local:
        query = query.filter(models.Places.name == local)

    # Add ORDER BY clause
    query = query.order_by(
        models.Registers.date.desc(),
        models.Registers.hours.desc(),
        models.Registers.minutes.desc(),
        models.Registers.seconds.desc()
    )
